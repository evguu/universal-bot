...
class VKServer(AbstractServer, KeyboardMixin):
    ...
    def send_message(self, user, message, reply_to_message_id=None):
        if isinstance(self, KeyboardMixin) and repr(user) in self.users_keyboards\
                and self.users_keyboards[repr(user)].options:
            vk.messages.send(
                message=message,
    ...
    def abstractify_request(self, raw_req):
        ...
        if raw_req["type"] == "confirmation":
            return AbstractServerRequest(User(self, 0, 0), "REBIND!")
